...
public  class  NewJFrame extends javax.swing.JFrame {
    ...
    public NewJFrame() {
        ...
        try {
            ...
            if (ack.equals("SERVER_ACK")) {
                System.out.println("podklycheno k servery.");
            }
    ...
    for (int row = 0; row < tModel.getRowCount(); row++) {
        ...
        final double[] totalResult = {0.0};  // Массив вместо AtomicReference
        ...
        for (int i = 0; i < 3; i++) {
            ...
            threads[i] = new Thread(() -> {
                ...
                for (double x = start; x < end; x += step) {
                    partialSum += Math.cos(x + step / 2) * step;
                }
                synchronized (totalResult) {
                    totalResult[0] += partialSum;  // Безопасное обновление
                }
            });
            threads[i].start();
        }
